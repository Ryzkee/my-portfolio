...
const roles = ["Front-End Development", "Back-End Development"];
...
useEffect(() => {
  ...
  if (charIndex < roles[roleIndex].length) {
    ...
    }, 2000); // Pause before switching to next role
  }
      ...
      <div className="w-full h-screen flex text-white" id={homeId}>
        <div className="w-[50%] h-full flex items-center">
          <div className="w-full h-1/3 flex items-center flex-col">
            ...
            <h1 className="w-[60%] text-5xl font-bold mb-2">
              I'm Ryan Jake Daz
            </h1>
            <p className="w-[60%] text-[15pt]">
              a{" "}
              <span className={`text-[#428fe2] font-bold `}>
                {
                  displayedText
                }
              </span>{" "}
            ...
            <div className="w-[60%] flex items-center mt-5 ml-10  h-[12%]">
              {socialIcons.map(({ icon: Icon, name, color }, index) => (
                <button
                  ...
                  style={{
                    backgroundColor:
                      hoveredIndex === index ? "transparent" : color,
                    border:
                      hoveredIndex === index ? `1px solid ${color}` : "none",
